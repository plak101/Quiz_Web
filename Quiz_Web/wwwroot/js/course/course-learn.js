...
async function loadFlashcardSet(contentId, flashcardSetId) {
    ...
    try {
        ...
        if (data.success && data.flashcards && data.flashcards.length > 0) {
            ...
            loadingDiv.innerHTML = `
                <i class="fas fa-exclamation-triangle fa-3x text-warning mb-3"></i>
                <p>B? flashcard này ch?a có th? nào.</p>
            `;
        }
    } catch (error) {
        ...
        loadingDiv.innerHTML = `
            <i class="fas fa-exclamation-circle fa-3x text-danger mb-3"></i>
            <p>Không th? t?i flashcards. Vui lòng th? l?i.</p>
        `;
    }
}
...
async function completeFlashcardSet(contentId) {
    ...
    try {
        ...
        if (data.success) {
            toastr.success('?ã hoàn thành b? flashcard!');
            ...
            if (completeBtn) {
                ...
                completeBtn.innerHTML = '<i class="bi bi-check-circle me-2"></i>?ã hoàn thành';
            }
        } else {
